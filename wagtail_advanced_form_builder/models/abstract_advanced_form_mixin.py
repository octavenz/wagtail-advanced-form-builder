    ...
    for condition in self.conditional_rules:
        ...
        if field_required:
            ...
            for cond in condition["conditions"]:
                ...
                all_conditions_passed = conditions_passed(
                    condition_rule, condition_value, condition_field_value
                )
...
class AbstractAdvancedFormMixin(models.Model):
    ...
    form = StreamField(
        [
            ...
            (
                consts.FIELD_TYPE_CHECKBOXES,
                CheckboxesFieldBlock(label=_("Checkboxes field")),
            ),
            (consts.FIELD_TYPE_CHECKBOX, CheckboxFieldBlock(label=_("Checkbox field"))),
            (
                consts.FIELD_TYPE_MULTI_SELECT,
                MultiSelectFieldBlock(label=_("Multiselect field")),
            ),
            (
                consts.FIELD_TYPE_MULTI_LINE,
                MultiLineFieldBlock(label=_("Multiline field")),
            ),
    ...
    thanks_page_title = models.CharField(
        ...
        blank=True,
        help_text=_(
            "The title for the thanks page. Defaults to the normal page title if left blank."
        ),
    )
            ...
            if rules:
                ...
                if len(conditions):
                    for condition in conditions:
                        del condition["id"]
                        del condition["type"]
                        condition["field_name"] = clean_form_field_name(
        ...
        for field in self.form.get_prep_value():
            ...
            if field["type"] == "form_snippet":
                ...
                form_field = self.form_field(
                    ...
                    display_side_by_side=field["value"].get(
                        "display_side_by_side", False
                    ),
                    display_checkbox_label=field["value"].get(
                        "display_checkbox_label", False
                    ),
                    html_value=html_value,
                    rule_action=rule_action,
                    name=field["value"].get("name", None),
                    toggled_label=field["value"].get("toggled_label", None),
                )
